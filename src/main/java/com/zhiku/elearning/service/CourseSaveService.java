...
import com.zhiku.elearning.util.md2Database.Md2Save;
import com.zhiku.elearning.view.ChapterProgressView;
...
public class CourseSaveService {
    ...
    public void saveContent(String filePath, int cid, String vid, List<ChapterProgressView> seqs) throws Exception {
        ...
        */
        ...
        CourseIndex courseIndex = indexTemplate.findByPrimaryKey(cid, vid);
        if (courseIndex == null) {//新课程，没有目录，创建新目录
            courseIndex = new CourseIndex();
            courseIndex.setCid(cid);
            courseIndex.setVid(vid);
            courseIndex.setCatalog(new ArrayList<>());
        }

        //合并目录
        courseIndex.setCatalog(ChildUtil.merge(courseIndex.getCatalog(), childList));
        //空章标题为null，改为""
        ...
        //储存//新加入的节和知识点sid均为负
        Map<Integer, Integer> kid2Newkid = indexTemplate.upset(courseIndex);
    ...
    public void delete(Integer cid, String vid, List<ChapterProgressView> seqs) {
        if (seqs == null || seqs.size() == 0) {
            ...
            for (Child chapter : courseIndex.getCatalog()) {
            ...
            if (xchapters.size() > 0) {
                courseIndex.setCatalog(xchapters);
                indexTemplate.upset(courseIndex);
            } else {
                indexTemplate.deleteByPrimaryKey(cid, vid);
            }
        }
    ...
    public CourseView preview(String filePath, Map<String, SectionContentView> sectionViewMap) throws Exception {
        ...
        courseIndex.setCid(0);
        courseIndex.setVid("V");
        courseIndex.setCatalog(chapterList);
        CourseView courseView1 = IndexTemplate.index2CourseView(courseIndex, course);
        //生成节视图对应
...
//        knowledgeView.setKnowledgeSection(child.getSid()); // fixme
    ...
    public List<ChapterProgressView> getProgress(Integer cid, String vid) {
        ...
        CourseIndex courseIndex = indexTemplate.getCatalog(cid, vid);
        if (courseIndex == null)
            return null;
        //记录序号
        List<ChapterProgressView> chapterProgressViews = new ArrayList<>();

        List<Child> chapters = courseIndex.getCatalog();
